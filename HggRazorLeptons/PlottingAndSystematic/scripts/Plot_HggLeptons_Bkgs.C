...
void PlotSMS( TString treeName = "HggRazor", TString model = "WH", TString var = "MR", int nbins = 80, float low = 0 , float high = 2000, TString x_label = "M_{R} [GeV]" )
{
  gROOT->Reset();
  ...
  //200_1 MassPoint
  TFile* f1 = new TFile("/Users/cmorgoth/Work/data/HggRazorLepton/V3p13_Apr132017/FastsimSignal/SMS-TChiWH_200_1_1pb_weighted.root", "read");
  TTree* t1 = (TTree*)f1->Get( treeName );
  TString plotCommand = var + Form( ">>tmp1(%d,%f,%f)", nbins, low, high );
  t1->Draw(plotCommand, "weight*btagCorrFactor*triggerEffSFWeight*photonEffSF*triggerEffWeight*(mGammaGamma >103. && mGammaGamma < 160. && pho1passIso == 1 && pho2passIso == 1 && pho1passEleVeto == 1 && pho2passEleVeto == 1 && abs(pho1Eta) <1.48 && abs(pho2Eta)<1.48 && (pho1Pt>40||pho2Pt>40)  && pho1Pt> 25. && pho2Pt>25. && MR>0)", "goff");
  ...
  //450_1 MassPoint
  TFile* f2 = new TFile("/Users/cmorgoth/Work/data/HggRazorLepton/V3p13_Apr132017/FastsimSignal/SMS-TChiWH_450_1_1pb_weighted.root", "read");
  TTree* t2 = (TTree*)f2->Get( treeName );
  plotCommand = var + Form( ">>tmp2(%d,%f,%f)", nbins, low, high );
  t2->Draw(plotCommand, "weight*btagCorrFactor*triggerEffSFWeight*photonEffSF*triggerEffWeight*(mGammaGamma >103. && mGammaGamma < 160. && pho1passIso == 1 && pho2passIso == 1 && pho1passEleVeto == 1 && pho2passEleVeto == 1 && abs(pho1Eta) <1.48 && abs(pho2Eta)<1.48 && (pho1Pt>40||pho2Pt>40)  && pho1Pt> 25. && pho2Pt>25. && MR>0)", "goff");
  ...
  //700_1 MassPoint
  TFile* f3 = new TFile("/Users/cmorgoth/Work/data/HggRazorLepton/V3p13_Apr132017/FastsimSignal/SMS-TChiWH_700_1_1pb_weighted.root", "read");
  TTree* t3 = (TTree*)f3->Get( treeName );
  plotCommand = var + Form( ">>tmp3(%d,%f,%f)", nbins, low, high );
  t3->Draw(plotCommand, "weight*btagCorrFactor*triggerEffSFWeight*photonEffSF*triggerEffWeight*(mGammaGamma >103. && mGammaGamma < 160. && pho1passIso == 1 && pho2passIso == 1 && pho1passEleVeto == 1 && pho2passEleVeto == 1 && abs(pho1Eta) <1.48 && abs(pho2Eta)<1.48 && (pho1Pt>40||pho2Pt>40)  && pho1Pt> 25. && pho2Pt>25. && MR>0)", "goff");
  ...
  c->SaveAs(model+"_"+var+"_Plot.pdf");
  c->SaveAs(model+"_"+var+"_Plot.png");
  c->SaveAs(model+"_"+var+"_Plot.C");
