...
RooWorkspace* MakeSignalBkgFit( TTree* treeData, TTree* treeSignal, TTree* treeSignal2, TTree* treeSMH, TTree* treeSMH2, TString mggName, bool isHighMass, TString name )
{
  //------------------------------------------------
  // C r e a t e   s i g n a l  s h a p e from TTree
  //------------------------------------------------
  ...
  RooDataSet data( "data", "", RooArgSet(mgg), RooFit::Import(*treeData) );
  //RooDataSet dataSignal( "dataSignal", "", RooArgSet(mgg), RooFit::Import(*treeSignal) );
  TTree* treeSignalMix = (TTree*)GetMixTree(treeSignal, treeSignal2, 0.46, 0.54);
  RooDataSet dataSignal( "dataSignal", "", RooArgSet(mgg), RooFit::Import(*treeSignalMix) );
  //RooDataSet dataSMH( "dataSMH", "", RooArgSet(mgg), RooFit::Import(*treeSMH) );
  TTree* treeSMHMix = (TTree*)GetMixTree(treeSMH, treeSMH2, 0.46, 0.54);
  RooDataSet dataSMH( "dataSMH", "", RooArgSet(mgg), RooFit::Import(*treeSMHMix) );
  //---------------------------------
  //D e f i n e   s i g n a l   P D F
  //---------------------------------
  int npoints = dataSignal.numEntries();
			    ...
			    TString sModel, TString f1, bool _signalOnly )
{
  ...
  TString year_wName = "weight"; 
  RooRealVar year_w( year_wName, "year_w", 0, 1);
  //--------------------------------
  //I m p or t i n g   D a t a
  //--------------------------------
  ...
  //RooDataSet dataSignal( "dataSignal", "", RooArgSet(mgg), RooFit::Import(*treeSignal) );
  RooDataSet dataSignal( "dataSignal", "", RooArgSet(mgg), RooFit::WeightVar(year_w), RooFit::Import(*treeSignal) );
  treeSMH->GetBranch("mGammaGamma")->SetName( mggName );
  //RooDataSet dataSMH( "dataSMH", "", RooArgSet(mgg), RooFit::Import(*treeSMH) );
  RooDataSet dataSMH( "dataSMH", "", RooArgSet(mgg), RooFit::WeightVar(year_w), RooFit::Import(*treeSMH) );
  //---------------------------------
  //D e f i n e   s i g n a l   P D F
  //---------------------------------
  int npoints = dataSignal.numEntries();
			    ...
			    TString sModel, TString f1, bool _signalOnly )
{
  ...
  TTree* treeSignalMix = (TTree*)GetMixTree(treeSignal, treeSignal2, 0.46, 0.54);
  treeSignalMix->GetBranch("mGammaGamma")->SetName( mggName );
  RooDataSet dataSignal( "dataSignal", "", RooArgSet(mgg), RooFit::Import(*treeSignalMix) );

  TTree* treeSMHMix = (TTree*)GetMixTree(treeSMH, treeSMH2, 0.46, 0.54);
  treeSMHMix->GetBranch("mGammaGamma")->SetName( mggName );
  RooDataSet dataSMH( "dataSMH", "", RooArgSet(mgg), RooFit::Import(*treeSMHMix) );
  //---------------------------------
  //D e f i n e   s i g n a l   P D F
  //---------------------------------
  ...
  */
  ...
  if ( !_signalOnly )
      ...
      for( int isys = 0; isys < signal_sys.size(); isys++ )
	{
	  if ( isys == 0 )
	    {
	      //if(signal_sys.at(isys)==0) ofs << "Signal_JES\t\t\t\tlnN\t\t1/1\t\t-\t\t-\n";
	      //else ofs << "Signal_JES\t\t\t\tlnN\t\t" << signal_sys.at(isys+1) << "/" << signal_sys.at(isys) << "\t\t-\t\t-\n";
	    }
	  else if ( isys == 2 )
	    {
	      ofs << "Signal_ISR\t\t\tlnN\t\t" << signal_sys.at(isys+1) << "/" << signal_sys.at(isys) << "\t\t-\t\t-\n";
...
//TFile* GetMixTree(TTree* tree1, TTree* tree2, float frac1, float frac2)
{ 
  float mgg;
  
  tree1->SetBranchAddress("mGammaGamma",&mgg);
  long int N1 = tree1->GetEntries();
  tree2->SetBranchAddress("mGammaGamma",&mgg);
  long int N2 = tree2->GetEntries();
  //long int N = 2*TMath::Min(N1,N2);
  long int N = TMath::Min(N1,N2);
  
  float sum = frac1+frac2;
  float ratio1 = frac1/sum;
  float ratio2 = frac2/sum;
  ...
  //std::cout<<"N1 = "<< N1 << ", frac1 = " << frac1 << ", ratio1 = " << ratio1 << "; "<<std::endl;
  //std::cout<<"N2 = "<< N2 << ", frac2 = " << frac2 << ", ratio2 = " << ratio2 << "; "<<std::endl;
  ...
  //std::generate(V2.begin(), V2.end(), [n = 0] () mutable { return n++; });
  std::generate(V1.begin(), V1.end(), RandomGenerator(N1));
  std::generate(V2.begin(), V2.end(), RandomGenerator(N2));
/*
  for (auto iv: V1) {
        std::cout << iv << " ";
  }
  std::cout << "\n";
*/
  //TString mixName = Form("mix_%d.root",rand());
  //TFile* fmix  = new TFile(mixName, "recreate");
  TTree* treeMix = new TTree("treeMix","treeMix");
  treeMix->Branch("mGammaGamma",&mgg);
  std::cout<<"fill tree"<<"  ";
  for (auto iv: V1) {
        tree1->GetEntry(iv);
        treeMix->Fill();
  ...
  std::cout<<"N = "<< treeMix->GetEntries() << "; "<<std::endl;
  treeMix->Write("HggRazorLeptons");
  //fmix->Close();
