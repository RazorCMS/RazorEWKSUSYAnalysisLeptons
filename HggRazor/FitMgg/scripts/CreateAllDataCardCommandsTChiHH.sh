...
extra_tag=Moriond_35p9ifb_Feb22_Summer16_ReMiniAOD_NewSigmaMoverM_Correction_PDFbugFixed_TChiHH
...
do
    ...
    echo ./CreateDataCardCommands" --inputCF="$indir1$name".txt --outputFile=MakeDataCards_"$extra_tag$name".sh --dataFile="$datafile" --smhFile="$smhfile" --signalFile="$sfile"/SMS-TChiHH_"$name"_tweaked_1pb_weighted.root --sModel="$name"_DataCards_"$extra_tag" --binningFile="$bfile --currentDir=$PWD
